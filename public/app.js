import {SCHEMA,ROLE_NAMES,JOB_FIELDS,clone,uid,blankRow,initialState,migrateState,switchPeriod,switchBrigade,brigadeName,employeeName,eligibleEmployees,participantAt,rowHasData,rowActive,calculatedHours,dayHours,monthHours,typeTotals,applyObject,applyRoster,validateRoster,setJobField,applyPaste,basicPay,dayPay} from './model.js?v=brigades-1';
import {renderMenu,handleMenuChange,handleMenuAction} from './menus.js?v=brigades-1';
...
let storageAvailable=true, state=initialState(), undoStack=[], view='schedule', toastTimer, renderedSchedule='';
...
const scheduleKey=()=>`${state.brigadeId}/${state.period}`;
...
function renderDay(day,index){
  ...
  let html=`<tr class="date-row ${index%2?'alt-day':''}"><td colspan="18"><div class="date-sticky"><button class="icon-button" data-collapse-day="${day.date}" aria-label="${collapsed?'Развернуть':'Свернуть'} ${dayTitle(day)}" aria-expanded="${!collapsed}">${collapsed?'▸':'▾'}</button><strong class="date-number">${dayTitle(day)}</strong><span class="date-weekday">${weekday}</span><span class="date-summary">${n} работ · ${num(dayHours(day))} ч${day.hoursOverride!==null?' (вручную)':''}</span><button class="day-add" data-add-rows="${day.date}">＋ 2 строки</button></div></td></tr>`;
  if(collapsed)return html;
  const rosterCell=(indices,cls)=>`<td class="shift-member ${cls}">${indices.map(i=>`<select data-shift-person="${i}" data-day="${day.date}" aria-label="${ROLE_NAMES[i]} смены, ${dayTitle(day)}">${employeeOptions(day.roster[i])}</select><select class="status-select" data-shift-status="${i}" data-day="${day.date}" aria-label="Статус: ${ROLE_NAMES[i]}, ${dayTitle(day)}">${options(state.statuses.map(s=>({value:s.code,label:s.code})),day.statuses[i])}</select>`).join('')}</td>`;
  html+=`<tr class="shift-row"><td class="shift-label">СОСТАВ<br>И СТАТУС</td>${rosterCell([0],'lead')}${rosterCell([1],'partner')}${rosterCell([2],'extra')}${rosterCell([3,4],'extra')}<td colspan="13" class="shift-tip">Состав смены можно изменить. В работу подставляются сотрудники со статусами РД и ДЕЖ.<br>В ячейке сотрудника можно убрать участие. Пустой расчетный итог возвращает автоматический расчет.</td></tr>`;
  day.rows.forEach((row,r)=>{
    const memberCell=(indices,cls)=>`<td class="job-member ${cls}">${indices.map(i=>{const auto=row.type&&['РД','ДЕЖ'].includes(day.statuses[i])?day.roster[i]:'';return `<select class="cell-input" ${attributes(day,row,`person${i}`)} title="Сотрудник из состава смены; можно убрать участие">${options([{value:'__auto',label:employeeName(state,auto)||'— автоматически'},...(day.roster[i]?[{value:day.roster[i],label:employeeName(state,day.roster[i])}]:[])],row.people[i]===null?'__auto':row.people[i],'— не участвует')}</select>`;}).join('')}</td>`;
    const missing=!row.type&&rowHasData(row);
    html+=`<tr class="job-row ${r%2?'alt-job':''}" data-job-id="${row.id}"><td class="time-cell">${input(day,row,'time',row.time,{type:'time'})}</td>${memberCell([0],'lead-cell')}${memberCell([1],'partner-cell')}${memberCell([2],'extra-cell')}${memberCell([3,4],'extra-cell')}<td>${select(day,row,'invoice',row.invoice,state.invoiceStates)}</td>`;
    if(r===0){html+=`<td rowspan="${day.rows.length}" class="money-cell">${input(day,row,'pay',dayPay(state,day),{type:'number',title:'Базовая сумма ставок. Очистите ячейку, чтобы вернуть расчет.'})}<span class="${day.pay===null?'sum-label':'manual-label'}">${day.pay===null?'по ставкам<br>без доплат':'вручную'}</span></td><td rowspan="${day.rows.length}" class="money-cell adjust-cell">${input(day,row,'adjustment',day.adjustment,{type:'number',placeholder:'—'})}<span class="sum-label">− штраф<br>+ премия</span></td><td rowspan="${day.rows.length}" class="shift-total">${input(day,row,'shiftHours',dayHours(day),{type:'number',title:'Сумма часов работ. Очистите ячейку, чтобы вернуть расчет.'})}<span class="${day.hoursOverride===null?'sum-label':'manual-label'}">${day.hoursOverride===null?'Σ часов':'вручную'}</span></td>`;}
    html+=`<td>${select(day,row,'objectId',row.objectId,state.objects.map(o=>({value:o.id,label:o.id+' · '+o.name})))}</td><td class="${missing?'invalid':''}">${select(day,row,'type',row.type,state.workTypes.map(t=>({value:t.code,label:t.code})))}${missing?'<span class="missing-marker">Выберите вид</span>':''}</td><td>${input(day,row,'hours',row.hours,{type:'number',placeholder:'—'})}</td><td>${textarea(day,row,'object',row.object,'object-title')}</td><td>${textarea(day,row,'phone',row.phone)}</td><td>${textarea(day,row,'objectNotes',row.objectNotes,'object-addition')}</td><td>${textarea(day,row,'task',row.task)}</td><td>${textarea(day,row,'notes',row.notes,'work-notes')}</td><td><div class="tech-wrap">${input(day,row,'tech',row.tech,{placeholder:'https://…'})}${/^https?:\/\//i.test(row.tech)?`<a href="${escape(row.tech)}" class="tech-link" target="_blank" rel="noopener noreferrer" aria-label="Открыть техбазу">↗</a>`:''}</div></td></tr>`;
  });return html;
}
function render(){
  if(renderedSchedule!==scheduleKey()){initializeScheduleView();renderedSchedule=scheduleKey();}
  $('period-month').value=state.period.slice(5);$('period-year').value=state.period.slice(0,4);
  const brigade=state.brigades.find(entry=>entry.id===state.brigadeId),code=brigadeName(state);
  $('brigade-select').innerHTML=options(state.brigades.map(entry=>({value:entry.id,label:entry.code})),state.brigadeId,null);
  $('brigade-select').title=brigade.label;
  $('brigade-breadcrumb').textContent=brigade.label;
  $('brigade-badge').textContent=code;
  document.title=`График — ${code} · ${periodTitle()}`;
  $('summary-period').textContent=`${code} · ${periodTitle()} · весь месяц`;
  $('schedule-table').caption.textContent=`Смены и работы ${code}, ${periodTitle()}`;
  ...
  const count=state.days.reduce((n,d)=>n+d.rows.filter(rowActive).length,0);
  $('total-description').textContent=`${count} работ · ${state.days.filter(d=>d.rows.some(rowActive)).length} смен с заданиями`;
  document.querySelectorAll('[data-roster]').forEach(el=>{el.innerHTML=employeeOptions(state.roster[Number(el.dataset.roster)]);});
  $('sheet-name').textContent=`${code} + ${employeeName(state,state.roster[0])||'Бригадир'} + ${employeeName(state,state.roster[1])||'Осн. напарник'}`;
  $('schedule-body').innerHTML=state.monthCollapsed?'':selectedDays().map(renderDay).join('');
  scroll.hidden=state.monthCollapsed;document.querySelector('.sheet-hint').hidden=state.monthCollapsed;
  $('collapse-month').textContent=state.monthCollapsed?'▸':'▾';$('collapse-month').setAttribute('aria-expanded',String(!state.monthCollapsed));$('collapse-month').setAttribute('aria-label',state.monthCollapsed?'Развернуть месяц':'Свернуть месяц');
  const allCollapsed=selectedDays().every(d=>state.collapsedDays.includes(d.date));$('collapse-days').textContent=allCollapsed?'Развернуть дни':'Свернуть дни';
  $('type-totals').innerHTML=typeTotals(state).map(([type,hours])=>`<div class="type-total" title="${escape(state.workTypes.find(t=>t.code===type)?.label||type)}"><span>${escape(type)}</span><strong>${num(hours)} ч</strong></div>`).join('')||'<span class="muted">Пока нет часов по видам работ</span>';
  $('undo').disabled=!undoStack.length;
  if(!storageAvailable)status('Хранилище недоступно или данные повреждены — экспортируйте изменения');
  scroll.scrollTop=top;scroll.scrollLeft=left;
  if(focusKey){const target=[...$('schedule-body').querySelectorAll('[data-field]')].find(el=>el.dataset.day===focusKey.day&&el.dataset.row===focusKey.row&&el.dataset.field===focusKey.field);target?.focus({preventScroll:true});}
  if(view!=='schedule')$('directory-view').innerHTML=renderMenu(state,view);
}
function navigate(next){view=next;$('schedule-view').hidden=next!=='schedule';$('directory-view').hidden=next==='schedule';document.querySelectorAll('[data-view]').forEach(b=>{b.classList.toggle('active',b.dataset.view===next);if(b.dataset.view===next)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');});if(next!=='schedule')$('directory-view').innerHTML=renderMenu(state,next);}
...
function changeSchedule(){
  ...
  const brigadeId=$('brigade-select').value;
  if(key===state.period&&brigadeId===state.brigadeId)return;
  try{
    if(storageAvailable){const latest=JSON.parse(localStorage.getItem(STORAGE)||'null');if(latest&&latest.revision!==state.revision){state=migrateState(latest);undoStack=[];render();toast('Данные обновлены из другой вкладки. Выберите бригаду и период повторно.');return;}}
    const next=clone(state);switchBrigade(next,brigadeId);switchPeriod(next,key);state=next;undoStack=[];
    const today=initializeScheduleView();renderedSchedule=scheduleKey();save();render();
    document.querySelector('.table-scroll').scrollTop=0;scrollToDay(today);
  }catch(error){toast(error.message||'Не удалось открыть график.');render();}
}
function scrollToDay(date){
  ...
  if(row)scroll.scrollTop+=row.getBoundingClientRect().top-scroll.getBoundingClientRect().top-$('schedule-table').tHead.getBoundingClientRect().height;
}
function login(){ $('login-view').hidden=true;$('app-view').hidden=false;const today=initializeScheduleView();navigate('schedule');render();scrollToDay(today); }
$('login-form').addEventListener('submit',event=>{event.preventDefault();if($('login').value==='admin'&&$('password').value==='admin'){try{sessionStorage.setItem(AUTH,'demo');}catch{}$('login-error').hidden=true;login();}else{$('login-error').textContent='Неверный логин или пароль. Для демо используйте admin / admin.';$('login-error').hidden=false;}});
...
$('period-month').addEventListener('change',changeSchedule);
$('period-year').addEventListener('change',changeSchedule);
$('brigade-select').addEventListener('change',changeSchedule);
...
$('schedule-body').addEventListener('change',event=>{const el=event.target,date=el.dataset.day;if(!date)return;
  ...
  else if(el.dataset.shiftPerson!==undefined){const i=Number(el.dataset.shiftPerson);mutate(s=>{const day=s.days.find(d=>d.date===date),roster=[...day.roster];roster[i]=el.value;validateRoster(s,roster);day.roster=roster;day.statuses[i]=el.value?'РД':'';day.rows.forEach(r=>r.people[i]=null);});}
  else if(el.dataset.shiftStatus!==undefined){const i=Number(el.dataset.shiftStatus);mutate(s=>{const day=s.days.find(d=>d.date===date);if(el.value&&!day.roster[i])throw Error('Сначала выберите сотрудника смены.');day.statuses[i]=el.value;});}
});
$('schedule-body').addEventListener('paste',event=>{const el=event.target;if(!el.dataset.field)return;const text=event.clipboardData.getData('text/plain');if(!text.includes('\t')&&!(el.tagName!=='TEXTAREA'&&text.trimEnd().includes('\n')))return;event.preventDefault();const day=state.days.find(d=>d.date===el.dataset.day),index=day.rows.findIndex(r=>r.id===el.dataset.row),column=JOB_FIELDS.indexOf(el.dataset.field==='person4'?'person3':el.dataset.field);if(column<0){toast('Вставьте диапазон начиная с основной ячейки строки.');return;}mutate(s=>applyPaste(s,day.date,index,column,text),'Диапазон вставлен.');});
...
$('export').addEventListener('click',async()=>{const button=$('export');button.disabled=true;button.textContent='Подготовка XLSX…';try{const snapshot=clone(state);const {downloadXlsx}=await import('./download-xlsx.js?v=brigades-1');await downloadXlsx(snapshot);toast('Книга XLSX готова: график, сотрудники и справочники.');}catch(error){toast(error.message||'Не удалось создать файл Excel.');}finally{button.disabled=false;button.textContent='↓ Экспорт XLSX';}});
